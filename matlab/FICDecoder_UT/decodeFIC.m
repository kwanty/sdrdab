...
global NFIBsPerFrame NCIFsPerFrame NSymbPerFIC  NFIBsPerCIF
  ...
  % Constants for FIC only
    ...
    if (Mode == 1)
    fid2 = fopen('../../data/ut/ficdec_in_mode_1.txt', 'a');
    for i = 1 : length(DataFIC)
        fprintf(fid2, '%f\n', DataFIC(i));
    end
    fclose(fid2);
    end
    if (Mode == 2)
    fid2 = fopen('../../data/ut/ficdec_in_mode_2.txt', 'a');
    for i = 1 : length(DataFIC)
        fprintf(fid2, '%f\n', DataFIC(i));
    end
    fclose(fid2);
    ...
    if (Mode == 3)
    fid2 = fopen('../../data/ut/ficdec_in_mode_3.txt', 'a');
    for i = 1 : length(DataFIC)
        fprintf(fid2, '%f\n', DataFIC(i));
    end
    fclose(fid2);
    ...
    if (Mode == 4)
    fid2 = fopen('../../data/ut/ficdec_in_mode_4.txt', 'a');
    for i = 1 : length(DataFIC)
        fprintf(fid2, '%f\n', DataFIC(i));
    end
    fclose(fid2);
  ...
  % Energy dispersal ------------------------------------------------------------------------
    ...
    for i = 1 : c
        for j = 1 : d
            e(d*(i-1)+j) =  DataEnerg(i,j);
        end
    end
    
    if (Mode == 1)
    fid2 = fopen('../../data/ut/ficdec_out_mode_1.txt', 'a');
    for i = 1 : c*d/8
        fprintf(fid2, '%d\n', bin2dec( reshape( char(e((i-1)*8+1:8*i)+'0'), 8, [])' )');
    end
    fclose(fid2);
    end
    if (Mode == 2)
    fid2 = fopen('../../data/ut/ficdec_out_mode_2.txt', 'a');
    for i = 1 : c*d/8
        fprintf(fid2, '%d\n', bin2dec( reshape( char(e((i-1)*8+1:8*i)+'0'), 8, [])' )');
    end
    fclose(fid2);
    ...
    if (Mode == 3)
    fid2 = fopen('../../data/ut/ficdec_out_mode_3.txt', 'a');
    for i = 1 : c*d/8
        fprintf(fid2, '%d\n', bin2dec( reshape( char(e((i-1)*8+1:8*i)+'0'), 8, [])' )');
    end
    fclose(fid2);
    ...
    if (Mode == 4)
    fid2 = fopen('../../data/ut/ficdec_out_mode_4.txt', 'a');
    for i = 1 : c*d/8
        fprintf(fid2, '%d\n', bin2dec( reshape( char(e((i-1)*8+1:8*i)+'0'), 8, [])' )');
    end
    fclose(fid2);
