...
#include "DataFeeder/data_feeder.h"
...
class Scheduler
{
	...
	uint8_t * audio_buffer_; ///< Buffor where DataDecoder writes and AudioDecoder reads

	uint8_t  demod_decod_buffer_multiplier_; ///< Size multiplier of demod_decod_buffer_ ( 1 - size of one deqpsk_block_size_)
	uint8_t sync_feeder_buffer_multiplier_; ///< Size multiplier of sync_feeder_buffer_ ( 1 - size of one sync_read_size_ )
	uint8_t  audio_buffer_multiplier_; ///< Size multiplier of audio_buffer_
	...
	size_t data_write_position_; ///< Position of data_write in sync_feeder_buffer_
	size_t sync_read_position_; ///<  Position of sync_read in sync_feeder_buffer_
	size_t demod_read_position_; ///< Position of demod_read in sync_feeder_buffer_
	size_t demod_write_position_; ///< Position of demod_write in demod_decod_buffer_
	size_t decod_read_position_; ///< Position of decod_read in demod_decod_buffer_
	size_t decod_write_position_; ///< Position of decod_write in audio_buffer_
	size_t audio_read_position_; ///< Position of audio_read in audio_buffer_
	...
	uint8_t fs_counter_;
	size_t sync_frame_size_; ///< Length between previous_null and current_null (used in calculating fs_drift)
	size_t previous_null_; ///< Start position of previous null (used in calculating fs_drift)
	size_t  current_null_; ///< Start position of current null (used in calculating fs_drift)
	uint8_t lost_frames_; ///< Number of lost_frames when data processing is to slow
	...
	uint8_t counter_; ///< Counter used to end program after specified number of frames in Process()

	uint8_t pointer_verbose_; ///< Verbose to buffer pointers debug
	size_t sync_feeder_counter_; ///< Used in buffer pointers debug
	size_t demod_counter_; ///< Used in buffer pointers debug
	size_t decod_counter_; ///< Used in buffer pointers debug

	uint8_t number_of_frames_for_calculate_fsdrift_; ///< For how many frames calculate fs_drift
	bool set_fs_drift_; ///< Value indicating when setting fs_drift is needed
	...
	pthread_t datafeeder_startprocessing_thread_; ///< DataFeeder StartProcessing() thread
	...
	pthread_t audiothread_; ///< AudioDecoder Process() thread
	...
	pthread_attr_t attr; ///< AudioDecoder Process() thread attribute
	audiodecoderData audiodecoder_data; ///< Structure for AudioDecoder Process() thread to exchange shared data

	bool station_found_; ///< Indicates when selected by user station was found

	float previous_fc_drift;
...
public:
	 ...
	 */
	void ParametersToSDR(uint8_t new_station);
	 ...
	 * @return next state
	 */
	state_t Sync( data_source_t data_source );
	...
	/**
	 * ConfStation method - deletes demod_decod_buffer for FIC, creates new demod_decod_buffer for proper station, creates audio_buffer_
	 * starts Demodulator Process to demodulate FIC, starts DataDecoder Process to decode FIC.
	 * @return next state
	 */
	state_t ConfStation();
	...
	/**
	 * CalculateAndSetDrifts method - calculates fs_drift, sets fs_drift and fc_drift
	 */
	...
	/**
	 * NOT COMPLETED, FOR FUTURE TESTING
	 */
	void SchedulerHardTest();
