...
//  printf("Callback!\n");
  ...
  float ns = 1000000.0 * static_cast<float>(block_size) / (2.0 * static_cast<float>(pto->GetFS()));
  ...
  if (0==pthread_mutex_timedlock(&(*ptctx->lock_buffer), &abs_time)){
    ...
    if (out_buffer!=pto->write_previous){

      pto->write_previous = out_buffer;
...
DataFeeder::DataFeeder(const char *dongle_name, int buf_n, size_t buf_s,
...
void DataFeeder::ReadAsync(void *data_needed){
	  rtlsdr_read_async(dev, AsyncCallback, data_needed, inner_buf_num,
		  inner_buff_size);
}

void DataFeeder::StopProcessing(void) {
  rtlsdr_cancel_async(dev);
  running = 0;
  return;
}

void DataFeeder::HandleDrifts(float fc_drift, float fs_drift) {
...
uint32_t DataFeeder::SetFC(uint32_t fc) {
...
uint32_t DataFeeder::SetFS(uint32_t fs) {
...
uint32_t DataFeeder::GetFS() {
  return rtlsdr_get_sample_rate(dev);
}

uint32_t DataFeeder::GetFC() {
  return rtlsdr_get_center_freq(dev);
}

int DataFeeder::SetGain(int gain) {
  if (gain < tuner_gain_count) {
    rtlsdr_set_tuner_gain(dev, tuner_gains[gain]);
    return rtlsdr_get_tuner_gain(dev);
  } else
    return -1;
}

bool DataFeeder::FromFile(void) {
  return false;
}

bool DataFeeder::FromDongle(void) {
  return true;
}

bool DataFeeder::EverythingOK(void){
	if (dev==NULL){
		if (verbose)
...
int DataFeeder::AGC(size_t clipped, size_t almost_clipped, size_t size) {
  if (!do_agc)
    return 0;
  if (debug) {
...
int DataFeeder::SetInitialGain(int gain) {
  // set manual gain
  ...
  float *tuner_linear_gains = new float[tuner_gain_count];
...
int DataFeeder::DongleInit(int dev_index, int samp_rate, int frequency,
  ...
  uint32_t real_samp_rate = SetFS(samp_rate);
  if (real_samp_rate == 0){
    if (verbose)
  ...
  uint32_t real_frequency = SetFC(frequency);
  if (real_frequency == 0){
    if (verbose)
...
int DataFeeder::VerboseDeviceSearch(const char *s) {
...
void DataFeeder::GetDongleList(std::list<std::string> * device_list)
{
