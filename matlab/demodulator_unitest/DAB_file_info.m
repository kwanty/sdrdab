    ...
    % ------------------------------------------------------------------------------------------
      ...
      if(Mode==1) % London Mode=1, only DAB
       ...
       %  SubChStartAdr = 96;  SubChSize = 96; Audiokbps = 128; Protection = 3; UEP = true;  % SubChID=5 DAB
          ...
          ReadFile = fopen(sprintf('../../data/%s', input_filename), 'rb');
...
if(0) ReadFile = fopen(sprintf('../../data/%s', input_filename), 'rb'); fs_file = 2.048e6;  sigtype=1;  fi=0;  coef = 0; end
...
if(1) ReadFile = fopen(sprintf('../../data/%s', input_filename), 'rb');  fs_file=2.048e6; sigtype=4; fi=0; coef=0.9; end

% #################################################################################################################
% DAB+ Krak�w

if(1) Audiokbps = 0;    Protection = ['3A']; UEP=false;  end % Channel #7 DAB+
