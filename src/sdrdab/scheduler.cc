                                   ...
                                   uint32_t carrier_freq, data_source_t data_source, Resampler2::resampling_type resample_quality)
{
    ...
    if ( data_source == DATA_FROM_FILE ) {
        datafeeder_ = new FileDataFeeder(file_type, dongle_or_file_name, internal_buffer_size, sample_rate, carrier_freq, 10 , resample_quality);
...
void Scheduler::CalculateAndSetDrifts() {
    ...
    if (data_source_ == DATA_FROM_DONGLE) {         //CalculateFsDriftFromFcDrift
        fs_drift_ = ( -estimated_fc_drift_ / carrier_frequency_ ) * 1000000; //tuuuuuuuuuuuuuuuuuuuuuuuutajaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
...
void Scheduler::Start(SchedulerConfig_t config)
{
    ...
    fileType_t file_type = config.file_type;
    ...
    state_ = Init(file_type, dongle_or_file_name, 4, 12 * 16384, config.sampling_rate, config.carrier_frequency, config.data_source, config.resample_quality);
