...
clearvars -except input_filename nr_test_files SuffixFileName;
...
% #############################
  ...
  if( DetectModeON )  % Mode = ?
      tic
      fprintf('\n**** mode generator started ****\n\n');
      NSamples = 2^19;
    % Load IQ file
      [ Signal ] = DataLoad_RT( ReadFile, NSamples, fs_file, fs, sigtype );
    % Detect frame beginings
      FrameStartpoints = DetFrameStart( Signal, PlotON, SuffixFileName );
    ...
    % Mode detection - other parameters depends on Mode and are calculated later
      [ Mode, NSampPerFrame, NSampPerNull, NSymbPerFrame, NSampPerSymb, NSampPerGuard, Nfft, NCarrPerSymb, FreqIntTabSize, Offset, NCIFsPerFrame, NSymbPerFIC ] = DetModeX( Signal, FrameStartpoints, SuffixFileName );
    % The main parameter, other values are based on it and recalculated after (done below) ! 
      toc; disp(' '); fprintf('Detected DAB Mode = %d\n\n', Mode);  
  end
...
while read == true
  ...
  % FRAME SYNCHRONIZATION --------------------------------------------------------------------
    if DABFrameFoundNr > nr_test_files
        break;
    end
    
    [Signal, OldData ] = DetFrameStartStream_RT( [OldData; NewData], ReadSize, CorrectTimeSynchro, CorrectDeltaFreq, CorrectADC, coef, PlotON, SuffixFileName);  % frame synchronization
