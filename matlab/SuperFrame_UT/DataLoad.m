...
function [Signal] = DataLoad( Filename, NSamples, fs_file, fs, sigtype )

if(sigtype==1)                                     % From Internet
   b = read_complex_binary(Filename, NSamples);    % Record1 Swiss -> 2 MSample,  9.8Sec, 20 091 724 Samples, 227.360MHz
                                                   % Record2 Swiss -> 2 MSample, 10.2Sec, 20 314 354 Samples, 194.064MHz
                                                   % Record3 Katowice --> 2.048 MSample
end

if(sigtype==2)                                     % USRP Ettus Research
   f = fopen(Filename, 'rb');                      % np. dab181-936MHz_at2MHz_cplx_short.dat
   
   if( Filename == 'dab181-936MHz_at2MHz_cplx_short.dat')    % for this recordings 
       b = fread( ReadFile, [2, 50000], 'int16' );           % skip first data (synchro problems)
   end
   
   b = fread(f,[2,NSamples],'int16');              %
   b  = b(1,:) + sqrt(-1)*b(2,:);                  %
end  
