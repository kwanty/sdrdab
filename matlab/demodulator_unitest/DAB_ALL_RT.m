...
clearvars -except input_filename nr_test_files SubChStartAdr SubChSize SuffixFileName;
...
% Testing four DAB modes - at present do not change
  TestMode = 0;         % 0/1 testing different DAB modes; 0 --> only Mode=1 (DAB+ Krakow)
  Mode = 1;             % 1/2/3/4: FOR TestMode=1 ONLY; otherwise Mode=1 
...
% Testing DAB+ only (e.g. DAB+ Krakow)  
  ...
  DetectModeON = 1;     % 0/1 example of DAB mode detection
...
% #############################
  ...
  if( DetectModeON )  % Mode = ?
    ...
    % The main parameter, other values are based on it and recalculated after (done below) ! 
      toc; disp(' '); disp('Detected DAB Mode = '); disp(Mode);  
  end
  ...
  % FRAME DECODING ---------------------------------------------------------------------------------
    if( ~isempty(Signal) & (DABFrameFoundNr>1) )  % first frame is without the freq sampling correction                                         % signal proceesing
        ...
        % ETSI pp. 153-157
          ...
          %%%%%%%%%%%%% generate input data %%%%%%%%%%%%%%%%%%%%%
            fp = fopen( sprintf('../../data/ut/demodulator_process_input_frame%d%s.txt', DABFrameNr, SuffixFileName), 'w' );
            NumOfSamples = length(Signal);
            data(1:2:2*NumOfSamples) = real(Signal(1:NumOfSamples));
            data(2:2:2*NumOfSamples) = imag(Signal(1:NumOfSamples));
            fprintf( fp, '%1.18f\n', data );
            fclose( fp );
          %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % ETSI pp. 157
          ...
          dqpsk_output = [DataFIC DataMSC];
  
          fp = fopen(sprintf('../../data/ut/demodulator_process_output_frame%d%s.txt', DABFrameNr, SuffixFileName), 'w');
          fprintf(fp, '%1.18f\n', dqpsk_output);
          fclose(fp);
          
         % Subtraction audio service 
          if Mode == 1
              number_cu_per_symbol = 48;
              number_symbols_per_cif = 18;
          elseif Mode == 2
              number_cu_per_symbol = 12;
              number_symbols_per_cif = 72;
          end
          symb_start_addr = floor(SubChStartAdr / number_cu_per_symbol) + 1;
          symb_end_addr = floor((SubChStartAdr + SubChSize - 1) / number_cu_per_symbol) + 1;
          DataMSC_sub = [];

          for cif = 0 : NCIFsPerFrame-1
              start_symb = NSymbPerFIC + cif * number_symbols_per_cif + symb_start_addr;
              end_symb = NSymbPerFIC + cif * number_symbols_per_cif + symb_end_addr;

              for f = start_symb : end_symb  % MSC DATA from OFDM symbols - first real parts, then imag parts 
                  DataMSC_sub = [DataMSC_sub real(DeintFFTFrame(f,:)) imag(DeintFFTFrame(f,:))];
              end
          end

          dqpsk_output_sub = [DataFIC DataMSC_sub];

          fp = fopen(sprintf('../../data/ut/demodulator_process_sub_output_frame%d%s.txt', DABFrameNr, SuffixFileName), 'w');
          fprintf(fp, '%1.18f\n', dqpsk_output_sub);
          fclose(fp);
        ...
        % Decode MSC (Main Service Channel ) - music, etc. ------------------------------------------------------  
          if( DetectMSCON )
              decodeMSC( DABFrameNr, DataMSC, SubChStartAdr, SubChSize, Audiokbps, Protection, UEP, trellis, RSpolynomial, SuperFrame2AAC, NBitsAfterTimeDepunct, DebugON );
          end
