...
function decodeMSCm1( DABFrameNr, DataMSC, SubChStartAdr, SubChSize, Audiokbps, Protection, UEP, trellis, RSpolynomial, SuperFrame2AAC, NBitsAfterTimeDepunct, DebugOn )
...
global CIFCount SuperFrameSync DataEnergChain au_start num_aus lan fid
   ...
   % Extracting data of one sub-channel requested by the user (CU=64bits = the smallest addressable unit)----
     ...
     fid2 = fopen(['../../data/ut/blocks1.txt'], 'at');
    ...
    if  (DABFrameNr == 15)
        for k = 1 : 11
            ...
            blocks = importdata('../../data/ut/blocks1.txt');     % todo: kwant (21.06.2016) should be loaded before for
    
            for i = 1 : NCIFsPerFrame
                for j = 1 : 64*SCS
                    block1(i, j) = blocks((i-1)*64*SCS+j+(k-1)*NCIFsPerFrame*64*SCS);
                end
            end

            for i = 1 : NCIFsPerFrame
                for j = 1 : 64*SCS
                    block2(i, j) = blocks((i-1)*64*SCS+j+NCIFsPerFrame*64*SCS+(k-1)*NCIFsPerFrame*64*SCS);
                end
            end

            for i = 1 : NCIFsPerFrame
                for j = 1 : 64*SCS
                    block3(i, j) = blocks((i-1)*64*SCS+j+2*NCIFsPerFrame*64*SCS+(k-1)*NCIFsPerFrame*64*SCS);
                end
            end

            for i = 1 : NCIFsPerFrame
                for j = 1 : 64*SCS
                    block4(i, j) = blocks((i-1)*64*SCS+j+3*NCIFsPerFrame*64*SCS+(k-1)*NCIFsPerFrame*64*SCS);
                end
            end
    
            for i = 1 : NCIFsPerFrame
                for j = 1 : 64*SCS
                    block5(i, j) = blocks((i-1)*64*SCS+j+4*NCIFsPerFrame*64*SCS+(k-1)*NCIFsPerFrame*64*SCS);
                end
            end
            ...
            if k == 1
                fp = fopen('../../data/ut/mscdec_in_m1.txt','at');
                ...
                fp = fopen('../../data/ut/mscdec_in_m1.txt','at');
                for i=215041:length(DataToInputFile)
                    fprintf( fp, '%f\n', DataToInputFile(i) );
                end
                fclose( fp );
            end
          ...
          % Super Frame Handling --------------------------------------------------------------------
            ...
            if(SuperFrame2AAC==1) % Super Frame decoding - below
               ...
               % CIFCount, 1+NBitsPerFrame*CIFCount, NDE+NBitsPerFrame*CIFCount, pause
                 ...
                 fid2 = fopen('../../data/ut/mscdec_out_m1.txt', 'a');
