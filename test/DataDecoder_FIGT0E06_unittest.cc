...
class FIGTest06  : public ::testing::Test{
    protected:
        FIGTest06(){
            //FIRST SET OF DATA
            input1Loaded = false;
        ...
        virtual void SetUp(){
            //FIRST SET OF DATA
            input1Loaded = MatlabIO::ReadData( input1, "./data/ut/T0E6Input.txt", length1 );
            output1Loaded = MatlabIO::ReadData( expectedOutput1, "./data/ut/T0E6FILS0Output.txt", lengthO);
            type1Loaded = MatlabIO::ReadData( &type1, "./data/ut/T0E6Type.txt", 1);
        }
        //FIRST SET OF DATA
        size_t length1;
        size_t lengthO;
...
TEST_F(FIGTest06, T0E6TEST) {
    ...
    EXPECT_EQ(static_cast<uint8_t>(6), output1) << "EXTENSION CHECK FAILED";
