...
% Testing DAB+ only (e.g. DAB+ Krakow)  
  ...
  DetectFICON =  1;     % 0/1 decoding FIC information about available services
  ...
  CorrectTimeSynchro = 1;  % 0/1 TRY TO CORRECT time synchro using known DF (delta freq)
  CorrectDeltaFreq   = 1;  % 0/1 TRY TO CORRECT df for the second time after improving Time Synchro
...
% CHOOSE DAB_IQ input file to be decoded, read its sampling freq, signal type, ... etc.
  ...
  ReadFile = fopen('../../data/Record3_katowice_iq.dat', 'rb');
...
% Example #2: PR Kraków with "noise"
...
% Mode      I       II     III    IV         % from ETSI EN300401, page 145, Table 38
  ...
  read = true;           % true = there are still DAB  waiting for processing
  OldData = [];          % DAB  still not-processed, waiting in the  buffer
...
% Buffer for time-interleaved MSC  (ETSI, pp. 137-142)
  ...
  % CHECK AMOUNT OF THE DATA IN THE BUFFER   
    ...
    if( MDAT < 2*NSampPerFrame)
      % READING NEXT DATA BLOCK FROM A DAB FILE --------------------------------------------------
        disp('RRRRRRRRRRRRRRRRRRRRRRRRRRRR Reading new  from disc ...');
        try    [NewData] = ComplexDataLoad( ReadFile, ReadSize, fs_file, fs, sigtype);  % load new block of input complex IQ 
        catch  read = false; break;                                                     %  are not enough (available)
        end
