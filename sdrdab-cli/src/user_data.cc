...
UserData::UserData()
	: DAB_plus_(),
	  bitrate_(),
	  station_id_(0),
	  station_label_("Not Available"),
	  time_(),
	  programme_type_(DataDecoder::InternationalProgrammeTable[0][0]),
	  coordinates_(),
	  stations(),
	  latitude_('N'),
	  longitude_('E'),
...
UserData::coordinate_t::coordinate_t(char direction)
...
void UserData::Set(UserFICData_t *data) {
		...
		data->programme_type_ < 32)
	{
		this->programme_type_ =
			DataDecoder::InternationalProgrammeTable[data->programme_type_][0];
	...
	if (data->validity_ & UserFICData_t::COORDINATES_VALID) {
		this->SetCoordinate(data->latitude_,
							UserData::coordinate_t::LATITUDE);
		this->SetCoordinate(data->longitude_,
							UserData::coordinate_t::LONGITUDE);
...
std::string UserData::DecodeEBULabel(const char label[16])
{
	char buf[4]; //buffer for translating function
	std::string retval;
	retval.reserve(16+8);

	for (int i = 0; i < 16; i++) {
		retval.append( DataDecoder::EBULatinToUTF8(label[i], buf) );
		if (label[i] == '\0')
			break;
	}
	return retval;
...
void UserData::SetCoordinate(int16_t coarse, coordinate_t::variety_t variety)
{
	coordinate_t UserData::*coord;
	double multiplier;
	if (variety == coordinate_t::LATITUDE) {
		coord = &UserData::latitude_;
...
void UserData::StringifyCoordinates(void) {
