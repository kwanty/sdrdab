...
class FIGTest021  : public ::testing::Test{
    protected:
        FIGTest021(){
            //FIRST SET OF DATA
            input1Loaded = false;
        ...
        virtual void SetUp(){
            //FIRST SET OF DATA
            input1Loaded = MatlabIO::ReadData( input1, "./data/ut/T0E21Input.txt", length1 );
            output1Loaded = MatlabIO::ReadData( expectedOutput1, "./data/ut/T0E21FOutput.txt", lengthO);
            type1Loaded = MatlabIO::ReadData( &type1, "./data/ut/T0E21Type.txt", 1);
        }
        //FIRST SET OF DATA
        size_t length1;
        size_t lengthO;
...
TEST_F(FIGTest021, T0E21TEST) {
    ...
    EXPECT_EQ(static_cast<uint8_t>(21), output1) << "EXTENSION CHECK FAILED";
